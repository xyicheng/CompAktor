...
class ActorSystemTest(unittest.TestCase):
    ...
    def test_actor_removal(self):
        ...
        sys.add_actor(a, "test")
        sys.delete_branch("1")
        self.assertEqual(a.get_state(), ActorState.TERMINATED,
                         "Actor was not Terminated")
        print("Printing Tree")
        sys.print_tree()
        asyncio.get_event_loop().run_until_complete(a.stop())
        sys.close()
    ...
    def test_branch_creation(self):
        ...
        self.assertEqual(a.get_state(), ActorState.TERMINATED,
                         "Actor was not Terminated")
        self.assertEqual(b.get_state(), ActorState.TERMINATED,
                         "Actor was not Terminated")
        self.assertEqual(c.get_state(), ActorState.TERMINATED,
                         "Actor was not Terminated")
        self.assertEqual(d.get_state(), ActorState.TERMINATED,
                         "Actor was not Terminated")
    ...
    def test_stress_add_two_levels(self):
        ...
        self.assertEqual(
            len([x for x in level_one if x.get_state() is ActorState.RUNNING]),
            0)
        self.assertEqual(len([x for x in level_one if x.get_state(
        ) is ActorState.TERMINATED]), len(level_one))
        self.assertEqual(
            len([x for x in level_two if x.get_state() is ActorState.RUNNING]),
            0)
        self.assertEqual(len([x for x in level_two if x.get_state(
        ) is ActorState.TERMINATED]), len(level_two))
...
class RoundRobinRouterTest(unittest.TestCase):
    ...
    def test_creation(self):
        ...
        sys.close()
        self.assertEqual(rr.get_state(), ActorState.TERMINATED,
                         "Router Not Terminated")
    ...
    def test_actor_addition(self):
        ...
        self.assertEqual(rr.get_num_actors(),
                         2,
                         "Actors Missing. Length {}".format(
                             rr.get_num_actors()))

        rr.remove_actor(a)
        self.assertEqual(rr.get_num_actors(), 1,
                         "Number of Actors Should be 1")
        asyncio.get_event_loop().run_until_complete(a.stop())
        sys.close()
        self.assertEqual(a.get_state(), ActorState.TERMINATED,
                         "Actor a Not Terminated")
        self.assertEqual(b.get_state(), ActorState.TERMINATED,
                         " Actor b Not Terminated")
        self.assertEqual(rr.get_state(), ActorState.TERMINATED,
                         "Router Not Terminated")
    ...
    def test_broadcast(self):
        ...
        self.assertEqual(rr.get_num_actors(), 2,
                         "Actors Missing. Length {}".format(
                             rr.get_num_actors()))
    ...
    def test_at_load(self):
        ...
        print(res)
        self.assertEqual(
            res, len(funcs), "Result {} is not {}".format(res, len(funcs)))
        sys.close()
        self.assertEqual(rr.get_state(), ActorState.TERMINATED,
                         "Router Not Terminated")
